---
title: "Patients exploration for CH evolution project"
...
tmn_blood <-
  read_rds(paste0(here::here(), 
                  "/data/processed_data",
                  "/CHevolution_all_patients_with_tmn_20251013.rds")) %>% 
...
# Data exploration
```{r}
breast_patients <- tmn_blood %>% 
  filter(primary_cancer_site_group == "Breast")
other_patients <- tmn_blood %>% 
  filter(primary_cancer_site_group != "Breast")
```

We included `r length(unique(other_patients$mrn))` non-breast patients with a minimum of `r min(other_patients$number_of_sample_bf_tmn, na.rm = TRUE)` samples, 
and `r length(unique(breast_patients$mrn))` breast patients with a minimum of `r min(breast_patients$number_of_sample_bf_tmn, na.rm = TRUE)` samples.
...
tmn_blood %>% 
  filter(sample_sequence_number == 1 |
           is.na(sample_sequence_number)) %>% 
  # mutate(number_of_sample_bf_tmn = factor(number_of_sample_bf_tmn, levels = c("1", "2", "3", "4", "5"))) %>%
...
pltly <- tmn_blood %>% 
  filter(number_of_sample_bf_tmn %in% c(1, 2)) %>% 
...
For patients with 3 samples minimum (breast + others). Breast patients point color is filled.
...
#             distinct(),
#           "Patient info selected for project _20251014.csv")
# write_csv(tmn_blood %>% select(mrn, sample_sequence_number, sample_id,
#                                sample_family_id, interval_sample_to_tmn_days,
#                                best_anatomic_site, sample_type),
#           "Patient samples info selected for project _20251014.csv")
