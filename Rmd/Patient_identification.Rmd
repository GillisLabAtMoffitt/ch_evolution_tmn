...
We included `r length(unique(breast_patients$mrn))` breast patients with a minimum of `r min(breast_patients$number_of_sample, na.rm = TRUE)` samples.
...
<!-- tmn_blood %>%  -->
<!--   filter(sample_sequence_number == 1) %>%  -->
<!--   ggplot(aes(x= blood_vs_tmn_sequence, y = interval_sample_to_tmn_days))+ -->
...
<!-- tmn_blood %>%  -->
<!--   filter(sample_sequence_number == 1) %>%  -->
<!--   ggplot(aes(x= blood_vs_tmn_sequence, y = interval_primary_cancer_to_samples_days))+ -->
